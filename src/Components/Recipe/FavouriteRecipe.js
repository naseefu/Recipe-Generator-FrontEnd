...
const FavouriteRecipePage = () => {
  ...
  useEffect(()=>{
    ...
    const fetchAllSuggestions=async()=>{
      ...
      setLoading(true)
      try{
        const response = await ApiServices.getAllRecipe(user.id)
  ...
  return (
    ...
    {loading?<div className="min-h-[600px] flex flex-col justify-center"><LoadingBar/></div>:
    <div className="p-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-bold text-gray-800 mb-8">Favourite Recipes</h1>
        ...
        {filteredRecipes.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-lg shadow-md">
            <p className="text-gray-500 text-xl">No favourite recipes, <span className="text-gray-500" onClick={()=>navigate("/recipe")}>try adding recipes</span></p>
          </div>
        ) : (
